...
from acquisition.saal import score_saal
from acquisition.utils import AcquisitionOutput, BaseAcquisition, compute_logit_mismatch_scores, tensor_stats


class SAALDualBStrategy(BaseAcquisition):
    """
    saal_dual_b:
      maximize c(x) subject to SAAL(x) >= kappa_b
    """
    ...
    def select(
        ...
        saal_time = time.perf_counter() - t_saal
        ...
        c_scores = compute_logit_mismatch_scores(
            ...
            device=device,
            progress_logger=progress_logger,
            progress_method_name="SAAL_DUAL_B_C",
        ).float()
        ...
        scoring_time = saal_time + c_time + filter_time
        ...
        if progress_logger is not None:
            ...
            progress_logger.log(
                (
                    "[SAAL_DUAL_B] timing "
                    f"saal={saal_time:.3f}s c_scores={c_time:.3f}s filter={filter_time:.3f}s "
        ...
        debug_data = {
            ...
            "c_score": c_scores.cpu().numpy(),
            "b_score": b_scores.cpu().numpy(),
        ...
        return AcquisitionOutput(
            ...
            extras={
                ...
                "c_stats": c_stats,
                "b_stats": b_stats,
                "timing_saal_scores_sec": float(saal_time),
