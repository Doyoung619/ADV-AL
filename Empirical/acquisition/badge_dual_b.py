...
class BADGEDualBStrategy(BaseAcquisition):
    """
    badge_dual_b (BADGE-constrained robustness selection)
    ...
    def select(
        ...
        c_scores = compute_logit_mismatch_scores(
            ...
            device=device,
            progress_logger=progress_logger,
            progress_method_name="BADGE_DUAL_B_C",
        ).float()
        ...
        scoring_time = badge_time + c_time + filter_time
        total_time = time.perf_counter() - total_t0

        if progress_logger is not None:
            progress_logger.log(
                (
                    "[BADGE_DUAL_B] c_stats "
            ...
            progress_logger.log(
                (
                    "[BADGE_DUAL_B] timing "
                    f"badge_embed={badge_time:.3f}s c_scores={c_time:.3f}s filter={filter_time:.3f}s "
                    f"selection={selection_time:.3f}s total={total_time:.3f}s"
                ),
                device=str(device),
            )
            progress_logger.log(
                f"[BADGE_DUAL_B] selected_indices={selected.astype(np.int64).tolist()}",
                device=str(device),
            )
