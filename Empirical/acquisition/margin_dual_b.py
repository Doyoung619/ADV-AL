...
from acquisition.margin import score_margin
from acquisition.utils import AcquisitionOutput, BaseAcquisition, compute_logit_mismatch_scores, tensor_stats


class MarginDualBStrategy(BaseAcquisition):
    """
    margin_dual_b:
      maximize c(x) subject to b(x) >= kappa_b

    Here:
      b(x) = -margin(x) = -(top1_prob - top2_prob),
    so larger b(x) means smaller margin (more uncertain).
    """
    ...
    def select(
        ...
        t_margin = time.perf_counter()
        b_scores = score_margin(
            model=model,
            unlabeled_loader=unlabeled_loader,
            device=device,
            progress_logger=progress_logger,
        ).float()
        margin_time = time.perf_counter() - t_margin
        ...
        c_scores = compute_logit_mismatch_scores(
            ...
            device=device,
            progress_logger=progress_logger,
            progress_method_name="MARGIN_DUAL_B_C",
        ).float()
        ...
        scoring_time = margin_time + c_time + filter_time
        total_time = time.perf_counter() - total_t0

        if progress_logger is not None:
            progress_logger.log(
                (
                    "[MARGIN_DUAL_B] c_stats "
            ...
            progress_logger.log(
                (
                    "[MARGIN_DUAL_B] b_stats(score=-margin) "
            ...
            progress_logger.log(
                (
                    "[MARGIN_DUAL_B] timing "
                    f"margin_scores={margin_time:.3f}s c_scores={c_time:.3f}s filter={filter_time:.3f}s "
        ...
        return AcquisitionOutput(
            ...
            extras={
                "method": "margin_dual_b",
                "score_definition_b": "b=-margin where margin=top1_prob-top2_prob",
                ...
                "c_stats": c_stats,
                "b_stats": b_stats,
                "timing_margin_scores_sec": float(margin_time),
