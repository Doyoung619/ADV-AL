...
class BALDDualBStrategy(BaseAcquisition):
    """
    bald_dual_b:
      maximize c(x) subject to BALD(x) >= kappa_b
    """
    ...
    def select(
        ...
        c_scores = compute_logit_mismatch_scores(
            ...
            device=device,
            progress_logger=progress_logger,
            progress_method_name="BALD_DUAL_B_C",
        ).float()
        ...
        scoring_time = bald_time + c_time + filter_time
        ...
        if progress_logger is not None:
            ...
            progress_logger.log(
                (
                    "[BALD_DUAL_B] timing "
                    f"bald={bald_time:.3f}s c_scores={c_time:.3f}s filter={filter_time:.3f}s "
