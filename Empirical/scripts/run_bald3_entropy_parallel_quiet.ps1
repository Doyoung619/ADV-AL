...
# BALD seed 3..9
$baldDir = Join-Path $root "bald"
New-Item -ItemType Directory -Force -Path $baldDir | Out-Null
foreach ($seed in 3..9) {
  $pureName = "exp__pure_seed${seed}_i500_b200_r10_pool5000"
  $pureDir = Join-Path $baldDir $pureName
  if (!(Test-RunCompleted -RunDir $pureDir)) {
    $args = @($common + @("--acquisition_method","bald","--seed","$seed","--output-dir",$baldDir,"--run-name",$pureName))
    $jobs.Add([pscustomobject]@{ algo="bald"; run_name=$pureName; args=$args })
  }
  foreach ($p in $percentiles) {
    $pTag = [int]($p * 100)
    $dualName = "exp__dual_b_p${pTag}_seed${seed}_i500_b200_r10_pool5000"
    $dualDir = Join-Path $baldDir $dualName
    if (!(Test-RunCompleted -RunDir $dualDir)) {
      $args = @($common + @("--acquisition_method","bald_dual_b","--dual-percentile","$p","--seed","$seed","--output-dir",$baldDir,"--run-name",$dualName))
      $jobs.Add([pscustomobject]@{ algo="bald"; run_name=$dualName; args=$args })
    }
  }
}

# ENTROPY seed 0..9
$entropyDir = Join-Path $root "entropy"
New-Item -ItemType Directory -Force -Path $entropyDir | Out-Null
foreach ($seed in 0..9) {
  $pureName = "exp__pure_seed${seed}_i500_b200_r10_pool5000"
  $pureDir = Join-Path $entropyDir $pureName
  if (!(Test-RunCompleted -RunDir $pureDir)) {
    $args = @($common + @("--acquisition_method","entropy","--seed","$seed","--output-dir",$entropyDir,"--run-name",$pureName))
    $jobs.Add([pscustomobject]@{ algo="entropy"; run_name=$pureName; args=$args })
  }
  foreach ($p in $percentiles) {
    $pTag = [int]($p * 100)
    $dualName = "exp__dual_b_p${pTag}_seed${seed}_i500_b200_r10_pool5000"
    $dualDir = Join-Path $entropyDir $dualName
    if (!(Test-RunCompleted -RunDir $dualDir)) {
      $args = @($common + @("--acquisition_method","entropy_dual_b","--dual-percentile","$p","--seed","$seed","--output-dir",$entropyDir,"--run-name",$dualName))
      $jobs.Add([pscustomobject]@{ algo="entropy"; run_name=$dualName; args=$args })
    }
  }
}

Write-Host "Remaining runs: $($jobs.Count)"
Write-Host "Max parallel: $MaxParallel | num-workers per run: $NumWorkers"
...
while ($cursor -lt $jobs.Count -or $running.Count -gt 0) {
  while ($cursor -lt $jobs.Count -and $running.Count -lt $MaxParallel) {
    ...
    $algoLogDir = Join-Path $logRoot $j.algo
    New-Item -ItemType Directory -Force -Path $algoLogDir | Out-Null
    $stdoutPath = Join-Path $algoLogDir "$($j.run_name).stdout.log"
    $stderrPath = Join-Path $algoLogDir "$($j.run_name).stderr.log"
...
if ($failed.Count -gt 0) {
  $failedPath = Join-Path $logRoot "failed_runs_bald3_entropy.json"
