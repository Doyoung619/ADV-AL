...
function Run-IfNeeded {
  ...
  $runDir = Join-Path $AlgoDir $RunName
...
foreach ($seed in 3..9) {
  ...
  Run-IfNeeded -AlgoDir $baldDir -RunName $pureName -CmdArgs $pureArgs

  foreach ($p in $percentiles) {
    $pTag = [int]($p * 100)
    $dualName = "exp__dual_b_p${pTag}_seed${seed}_i500_b200_r10_pool5000"
    $dualArgs = @($common + @(
      "--acquisition_method", "bald_dual_b",
      "--dual-percentile", "$p",
      "--seed", "$seed",
      "--output-dir", $baldDir,
      "--run-name", $dualName
    ))
    Run-IfNeeded -AlgoDir $baldDir -RunName $dualName -CmdArgs $dualArgs
  }
}
...
foreach ($seed in 0..9) {
  $pureName = "exp__pure_seed${seed}_i500_b200_r10_pool5000"
  $pureArgs = @($common + @(
    "--acquisition_method", "entropy",
    "--seed", "$seed",
    "--output-dir", $entropyDir,
    "--run-name", $pureName
  ))
  Run-IfNeeded -AlgoDir $entropyDir -RunName $pureName -CmdArgs $pureArgs

  foreach ($p in $percentiles) {
    $pTag = [int]($p * 100)
    $dualName = "exp__dual_b_p${pTag}_seed${seed}_i500_b200_r10_pool5000"
    $dualArgs = @($common + @(
      "--acquisition_method", "entropy_dual_b",
      "--dual-percentile", "$p",
      "--seed", "$seed",
      "--output-dir", $entropyDir,
      "--run-name", $dualName
    ))
    Run-IfNeeded -AlgoDir $entropyDir -RunName $dualName -CmdArgs $dualArgs
  }
}

Write-Host "Completed target jobs: BALD(seed 3..9) + ENTROPY(seed 0..9)."
