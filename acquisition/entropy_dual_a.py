...
class EntropyDualAStrategy(BaseAcquisition):
    """
    entropy_dual_a:
      maximize Entropy(x) subject to c(x) >= kappa_c
    """
    ...
    def select(
        ...
        c_scores = compute_logit_mismatch_scores(
            ...
            progress_method_name="ENTROPY_DUAL_A_C",
        ).float()
        ...
        scoring_time = c_time + entropy_time + filter_time
        total_time = time.perf_counter() - total_t0

        if progress_logger is not None:
            progress_logger.log(
                (
                    "[ENTROPY_DUAL_A] c_stats "
            ...
            progress_logger.log(
                (
                    "[ENTROPY_DUAL_A] timing "
                    f"c_scores={c_time:.3f}s entropy={entropy_time:.3f}s filter={filter_time:.3f}s "
