...
foreach ($seed in 3..9) {
  ...
  Run-IfNeeded -AlgoDir $baldDir -RunName $pureName -Args $pureArgs
  ...
  foreach ($p in $percentiles) {
    ...
    Run-IfNeeded -AlgoDir $baldDir -RunName $dualName -Args $dualArgs
  }
}
...
foreach ($seed in 0..9) {
  ...
  Run-IfNeeded -AlgoDir $entropyDir -RunName $pureName -Args $pureArgs
  ...
  foreach ($p in $percentiles) {
    ...
    Run-IfNeeded -AlgoDir $entropyDir -RunName $dualName -Args $dualArgs
  }
}

Write-Host "Resume run finished: BALD(seed 3..9) + ENTROPY(seed 0..9)."
