...
$root = "./output_Experiment1/margin"
...
foreach ($seed in 0..9) {
  ...
  if ($SkipCompleted -and (Test-RunCompleted -RunDir $pureDir)) {
    ...
    $args = @($common + @(
      "--acquisition_method", "margin",
      "--seed", "$seed",
      "--run-name", $pureName
    ))
    $jobs.Add([pscustomobject]@{ run_name=$pureName; method="margin"; args=$args })
  }
  ...
  foreach ($p in $percentiles) {
    ...
    if ($SkipCompleted -and (Test-RunCompleted -RunDir $dualDir)) {
      ...
      $args = @($common + @(
        "--acquisition_method", "margin_dual_b",
        "--dual-percentile", "$p",
        "--seed", "$seed",
        "--run-name", $dualName
      ))
      $jobs.Add([pscustomobject]@{ run_name=$dualName; method="margin_dual_b"; args=$args })
    }
  }
}
