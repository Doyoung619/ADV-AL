...
LAUNCH_ROOT="${LAUNCH_ROOT:-${EXPERIMENT_ROOT}/sbatch_corr_residual_p0_p10_p25_seed0_9_12gpu_bigqos}"
...
PARTITION_SPECS="${PARTITION_SPECS:-dell_rtx3090:4,suma_a6000:4,suma_rtx4090:4}"
QOS="${QOS:-big_qos}"
...
write_sbatch() {
  local partition="$1"
  local task_count="$2"
  local script_path="${LAUNCH_ROOT}/sbatch_${partition}_${task_count}gpu.sh"
  cat > "${script_path}" <<EOF
#!/bin/bash
#SBATCH -p ${partition}
#SBATCH -q ${QOS}
#SBATCH -J corr_res_${partition}
#SBATCH --array=0-$((task_count - 1))
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=12
#SBATCH --mem=90G
#SBATCH --time=3-00:00:00
#SBATCH --output=${LAUNCH_ROOT}/slurm/${partition}_%A_%a.out
#SBATCH --error=${LAUNCH_ROOT}/slurm/${partition}_%A_%a.err
...
sbatch_scripts=()
IFS=',' read -r -a PARTITION_ARRAY <<< "${PARTITION_SPECS}"
for spec in "${PARTITION_ARRAY[@]}"; do
  partition="${spec%%:*}"
  task_count="${spec##*:}"
  if [[ -z "${partition}" || -z "${task_count}" || "${task_count}" -le 0 ]]; then
    echo "Invalid PARTITION_SPECS entry: ${spec}" >&2
    exit 2
  fi
  sbatch_scripts+=("$(write_sbatch "${partition}" "${task_count}")")
done
...
for script in "${sbatch_scripts[@]}"; do
  echo "sbatch \"${script}\"" >> "${LAUNCH_ROOT}/submit.sh"
done
cat >> "${LAUNCH_ROOT}/submit.sh" <<'EOF'
echo "submitted corr-residual CIFAR100 ResNet18 queue"
EOF
...
echo "Partitions:"
echo "  ${PARTITION_SPECS} with qos=${QOS}"
echo
echo "One-shot command:"
echo "  PROCS_PER_GPU=${PROCS_PER_GPU} NUM_WORKERS=${NUM_WORKERS} CONDA_ENV=${CONDA_ENV} PARTITION_SPECS=${PARTITION_SPECS} QOS=${QOS} bash ${ROOT_DIR}/scripts/run_corr_residual_cifar100_resnet18_sbatch_12gpu.sh && bash ${LAUNCH_ROOT}/submit.sh"
